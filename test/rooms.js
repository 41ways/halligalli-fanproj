...
(async () => {
  ...
  await check('방을 만들면 목록을 보는 소켓에 뜬다', async () => {
    a = sock();
    game.handle(a, { t: 'create', name: '가나', mode: 'basic' });
    codeA = codeOf(a);
    await sleep(FLUSH);
  ...
  await check('익스트림 방은 모드가 같이 뜬다', async () => {
    const e = sock();
    game.handle(e, { t: 'create', name: '익스', mode: 'extreme' });
    const codeE = codeOf(e);
    await sleep(FLUSH);
    assert.strictEqual(lastList(w).find(x => x.code === codeE).mode, 'extreme');
